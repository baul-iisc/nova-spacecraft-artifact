...
// MMACD(md, ms1, ms2): md += ms1 * ms2^T (3x3 output-stationary MAC, double precision)
// All three operands are matrix registers (encoded in rd, rs1, rs2 fields)
// Second operand is accessed transposed (output-stationary systolic array pattern)
{
  int md  = insn.rd();
  int ms1 = insn.rs1();
  int ms2 = insn.rs2();
