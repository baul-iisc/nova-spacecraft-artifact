 ...
 *
 * Uses 3×3 SYS_MMACD tiles. MATRIX_SIZE=9 is an exact multiple of 3,
...
/* ================================================================
 * HW TILED ELEMENT-WISE ADD / SUBTRACT
 * Processes 9 elements (one 3×3 tile) per iteration.
 * SW tail for remainder.
 * ================================================================ */
 ...
 * MATRIX_SIZE=9 is an exact multiple of 3.
 * ================================================================ */

// Matrix multiply: C = A × B (9×9)
void matrix_multiply(Matrix *result, Matrix *a, Matrix *b) {
 Matrix temp;
 memset(&temp, 0, sizeof(Matrix));
 hw_matmul(&a->data[0][0], &b->data[0][0], &temp.data[0][0],
 MATRIX_SIZE, MATRIX_SIZE, MATRIX_SIZE);
 memcpy(result, &temp, sizeof(Matrix));
}

// Matrix add: C = A + B (9×9, 81 elements = 9 tiles of 9)
void matrix_add(Matrix *result, Matrix *a, Matrix *b) {
 hw_elem_add(&a->data[0][0], &b->data[0][0], &result->data[0][0],
 MATRIX_SIZE * MATRIX_SIZE);
}

// Matrix subtract: C = A - B (9×9)
void matrix_subtract(Matrix *result, Matrix *a, Matrix *b) {
 hw_elem_sub(&a->data[0][0], &b->data[0][0], &result->data[0][0],
 MATRIX_SIZE * MATRIX_SIZE);
}

// Matrix scale: C = A * scalar (no HW equivalent — pure SW)
void matrix_scale(Matrix *result, Matrix *a, double scalar) {
...
/* ================================================================
 * 9×9 MATRIX INVERSE via two-level block Schur complement
 *
 * Partition 9×9 as:
 * M = [A(6×6) B(6×3)]
 * [C(3×6) D(3×3)]
 *
 * Level 1: inv_6x6(A) using block Schur with two MINVD calls
 * Level 2: S = D – C·A⁻¹·B (3×3 Schur complement)
 * S⁻¹ via MINVD
 *
 * M⁻¹ = [ A⁻¹ + A⁻¹·B·S⁻¹·C·A⁻¹ –A⁻¹·B·S⁻¹ ]
 * [ –S⁻¹·C·A⁻¹ S⁻¹ ]
 *
 * Total MINVD calls: 3 (two inside inv_6x6 + one for S)
 * ================================================================ */
int matrix_inverse(Matrix *result, Matrix *a) {
...
Quaternion quaternion_from_axis_angle(Vector3 axis, double angle) {
 ...
 double sin_half = hw_sin(half_angle);

 q.w = hw_cos(half_angle);
 q.x = axis.x * sin_half;
 q.y = axis.y * sin_half;
 q.z = axis.z * sin_half;
...
Vector3 compute_clohessy_wiltshire(SpacecraftState *chaser, SpacecraftState *target, double mean_motion) {
 ...
 double nt = n * tf;
 double c = hw_cos(nt), s = hw_sin(nt);
...
void attitude_control(SpacecraftState *chaser, SpacecraftState *target,
 ...
 double angle = hw_acos(fmax(-1.0, fmin(1.0, vector_dot(x_axis, direction))));
 ...
 double angle_error = 2.0 * hw_acos(fmax(-1.0, fmin(1.0, q_error.w)));
 Vector3 axis_error;
...
int check_docking_conditions(SpacecraftState *chaser, SpacecraftState *target) {
 ...
 lateral_speed < 0.032 &&
 alignment > hw_cos(DOCKING_ANGLE_THRESHOLD)) {
