...
void initializeSystem() {
 ...
 starCatalog[i].direction.x = sin(phi) * cos(theta);
 starCatalog[i].direction.y = sin(phi) * sin(theta);
 starCatalog[i].direction.z = cos(phi);
...
void matchStars(Star* detectedStars, int starCount) {
 ...
 // Calculate angle
 detectedAngles[i][j] = acos(dotProduct);
 ...
 // Calculate angle
 catalogAngles[i][j] = acos(dotProduct);
 ...
 *
 * Uses 3×3 SYS_MMACD tiles. STATE_DIM=18 is an exact multiple of 3,
 * so no boundary-padding is needed.
 * ================================================================ */
...
/* ================================================================
 * BLOCK INVERSE HIERARCHY for 18×18
 *
 * inv_6x6: [3×3|3×3; 3×3|3×3] → 2 MINVD
 * inv_9x9: [6×6|6×3; 3×6|3×3] → 3 MINVD (2 inside inv_6x6 + 1)
 * inv_18x18: [9×9|9×9; 9×9|9×9] → 6 MINVD (3+3)
 * ================================================================ */

/* 6×6 inverse via block Schur complement */
 ...
 * STATE_DIM=18, MEASUREMENT_DIM=18 — both exact multiples of 3.
 * ================================================================ */

// Matrix multiply: C = A × B (18×18)
void matrixMultiply(Matrix* A, Matrix* B, Matrix* result) {
 memset(result, 0, sizeof(Matrix));
 hw_matmul(&A->data[0][0], &B->data[0][0], &result->data[0][0],
 STATE_DIM, STATE_DIM, STATE_DIM);
}

// Matrix-vector multiply (no HW — pure SW)
void matrixVectorMultiply(Matrix* A, Vector* v, Vector* result) {
 for (int i = 0; i < STATE_DIM; i++) {
 result->data[i] = 0.0;
 }
...
// Matrix add: C = A + B (18×18, 324 elements = 36 tiles of 9)
void matrixAdd(Matrix* A, Matrix* B, Matrix* result) {
 hw_elem_add(&A->data[0][0], &B->data[0][0], &result->data[0][0],
 STATE_DIM * STATE_DIM);
}

// Matrix subtract: C = A - B (18×18)
void matrixSubtract(Matrix* A, Matrix* B, Matrix* result) {
 hw_elem_sub(&A->data[0][0], &B->data[0][0], &result->data[0][0],
 STATE_DIM * STATE_DIM);
}

// Matrix transpose (no HW transpose instruction — pure SW)
void matrixTranspose(Matrix* A, Matrix* result) {
 for (int i = 0; i < STATE_DIM; i++) {
 for (int j = 0; j < STATE_DIM; j++) {
...
/* ================================================================
 * 18×18 MATRIX INVERSE via block Schur complement
 *
 * Partition as [A(9×9) B(9×9)]
 * [C(9×9) D(9×9)]
 *
 * inv_9x9(A): 3 MINVD
 * Schur S = D – C·A⁻¹·B → inv_9x9(S): 3 MINVD
 * Total: 6 MINVD calls
 * ================================================================ */
void matrixInverse(Matrix* A, Matrix* result) {
 const int n = STATE_DIM; /* 18 */
 const double *M = &A->data[0][0];
 double *Minv = &result->data[0][0];

 /* Extract blocks from 18×18 matrix */
 double A_blk[81]; /* 9×9 top-left */
 double B_blk[81]; /* 9×9 top-right */
 double C_blk[81]; /* 9×9 bottom-left */
 double D_blk[81]; /* 9×9 bottom-right */

 for (int i = 0; i < 9; i++)
 for (int j = 0; j < 9; j++) {
 A_blk[i*9+j] = M[i*n + j];
 B_blk[i*9+j] = M[i*n + (j+9)];
 C_blk[i*9+j] = M[(i+9)*n + j];
 D_blk[i*9+j] = M[(i+9)*n + (j+9)];
 }
 ...
 /* Step 2: CA_inv = C(9×9) · A_inv(9×9) → (9×9) */
 double CA_inv[81];
 hw_matmul(C_blk, A_inv, CA_inv, 9, 9, 9);

 /* Step 3: CA_inv_B = CA_inv(9×9) · B(9×9) → (9×9) */
 double CA_inv_B[81];
 hw_matmul(CA_inv, B_blk, CA_inv_B, 9, 9, 9);

 /* Step 4: S = D - CA_inv_B (9×9 Schur complement) */
 double S[81];
 hw_elem_sub(D_blk, CA_inv_B, S, 81);

 /* Step 5: S_inv = inv(S) — 9×9 via inv_9x9 (3 MINVD) */
 double S_inv[81];
 inv_9x9(S, S_inv);

 /* Step 6: A_inv_B = A_inv(9×9) · B(9×9) → (9×9) */
 double A_inv_B[81];
 hw_matmul(A_inv, B_blk, A_inv_B, 9, 9, 9);

 /* Step 7: A_inv_B_Sinv = A_inv_B(9×9) · S_inv(9×9) → (9×9) */
 double A_inv_B_Sinv[81];
 hw_matmul(A_inv_B, S_inv, A_inv_B_Sinv, 9, 9, 9);

 /* Step 8: Sinv_CA_inv = S_inv(9×9) · CA_inv(9×9) → (9×9) */
 double Sinv_CA_inv[81];
 hw_matmul(S_inv, CA_inv, Sinv_CA_inv, 9, 9, 9);

 /* Step 9: correction = A_inv_B_Sinv(9×9) · CA_inv(9×9) → (9×9) */
 double correction[81];
 hw_matmul(A_inv_B_Sinv, CA_inv, correction, 9, 9, 9);

 /* Step 10: top_left = A_inv + correction → (9×9) */
 double top_left[81];
 hw_elem_add(A_inv, correction, top_left, 81);

 /* Assemble 18×18 result */
 for (int i = 0; i < 9; i++)
 for (int j = 0; j < 9; j++) {
 Minv[i*n + j] = top_left[i*9+j];
 Minv[i*n + (j+9)] = -A_inv_B_Sinv[i*9+j];
 Minv[(i+9)*n + j] = -Sinv_CA_inv[i*9+j];
 Minv[(i+9)*n + (j+9)] = S_inv[i*9+j];
 }
}
