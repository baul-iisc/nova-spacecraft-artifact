 ...
 * Target : RISC-V (RV64GC) with CORDIC Accelerator
...
void matrix_multiply(double *A, double *B, double *C, int rows_a, int cols_a, int cols_b) {
 ...
 /* Matrix multiplication algorithm - naive implementation
 * Later this can be replaced with hardware matrix operations
 */
 for (i = 0; i < rows_a; i++) {
 for (j = 0; j < cols_b; j++) {
 for (k = 0; k < cols_a; k++) {
 ...
 * Returns 0 if successful, -1 if matrix is singular
 */
int matrix_inverse(double *A, double *Ainv, int n) {
 int i, j, k;
 int max_i;
 ...
 EciEcefRotation *rot)
{
 ...
 double zeta_A = ( 2.5976176 + 2306.0809506*T + 0.3019015*T2
 + 0.0179663*T3) * AS2RAD;
 double z_A = (-2.5976176 + 2306.0803226*T + 1.0947790*T2
 + 0.0182273*T3) * AS2RAD;
 double theta_A = (2004.1917476*T - 0.4269353*T2
 - 0.0418251*T3) * AS2RAD;

 /* P = R₃(−z_A) · R₂(θ_A) · R₃(−ζ_A) */
 double cz = hw_cos(z_A), sz = hw_sin(z_A);
 double ct = hw_cos(theta_A), st = hw_sin(theta_A);
 double czeta = hw_cos(zeta_A), szeta = hw_sin(zeta_A);
 ...
 * total nutation amplitude, sufficient for GNSS L5 OD. */
 ...
 double eps_A = eps_0 - (46.836769*T - 0.0001831*T2
 + 0.00200340*T3) * AS2RAD;
 ...
 double dpsi = (-17.2064161 * hw_sin(Om)
 - 1.3170906 * hw_sin(2.0*(F - D + Om))
 - 0.2276413 * hw_sin(2.0*(F + Om))
 + 0.2074554 * hw_sin(2.0*Om)) * AS2RAD;

 double deps = ( 9.2052331 * hw_cos(Om)
 + 0.5730336 * hw_cos(2.0*(F - D + Om))
 + 0.0978459 * hw_cos(2.0*(F + Om))
 - 0.0897492 * hw_cos(2.0*Om)) * AS2RAD;

 /* N = R₁(−ε_total) · R₃(−Δψ) · R₁(ε_A) */
 double eps_total = eps_A + deps;
 double ce = hw_cos(eps_A), se = hw_sin(eps_A);
 double cet = hw_cos(eps_total), set = hw_sin(eps_total);
 double cdp = hw_cos(dpsi), sdp = hw_sin(dpsi);
 ...
 * GAST = GMST + equation of equinoxes */
 ...
 double eq_equinox = dpsi * hw_cos(eps_A)
 + 0.00264096 * AS2RAD * hw_sin(Om)
 + 0.00006352 * AS2RAD * hw_sin(2.0 * Om);
 double gast = gmst_rad + eq_equinox;

 double cg = hw_cos(gast), sg = hw_sin(gast);
 ...
 * x_p, y_p from IERS Bulletin B (updated ~weekly by ground). */
 ...
 double cxp = hw_cos(xp), sxp = hw_sin(xp);
 double cyp = hw_cos(yp), syp = hw_sin(yp);
 ...
 *alt = fabs(ecef[2]) - b;
 ...
 double theta = hw_atan2(ecef[2] * a, p * b);

 *lon = hw_atan2(ecef[1], ecef[0]);
 *lat = hw_atan2(ecef[2] + ep2 * b * pow(hw_sin(theta), 3),
 p - e2 * a * pow(hw_cos(theta), 3));

 double sin_lat = hw_sin(*lat);
...
void ecef_to_enu(double ecef[3], double ref_ecef[3], double enu[3]) {
 ...
 double sin_lat = hw_sin(lat);
 double cos_lat = hw_cos(lat);
 double sin_lon = hw_sin(lon);
 double cos_lon = hw_cos(lon);
...
void orbital_elements_to_state_vector(SatelliteEphemeris *eph, double time, double state_vector[6]) {
 ...
 E_new = E - (E - e * hw_sin(E) - M) / (1.0 - e * hw_cos(E));
 ...
 double nu = 2.0 * hw_atan2(sqrt(1.0 + e) * hw_sin(E / 2.0), sqrt(1.0 - e) * hw_cos(E / 2.0));

 /* Compute position in orbital plane */
 double r = a * (1.0 - e * hw_cos(E));
 double x_orb = r * hw_cos(nu);
 double y_orb = r * hw_sin(nu);

 /* Compute velocity in orbital plane */
 double p = a * (1.0 - e * e);
 double sqrt_mu_over_p = sqrt(EARTH_MU / p);
 double vx_orb = -sqrt_mu_over_p * hw_sin(nu);
 double vy_orb = sqrt_mu_over_p * (e + hw_cos(nu));

 /* Rotation matrices for orbital plane to ECI */
 double cos_Omega = hw_cos(Omega);
 double sin_Omega = hw_sin(Omega);
 double cos_i = hw_cos(i);
 double sin_i = hw_sin(i);
 double cos_omega = hw_cos(omega);
 double sin_omega = hw_sin(omega);
...
void state_vector_to_orbital_elements(double state_vector[6], SatelliteEphemeris *eph) {
 ...
 /* Inclination */
 eph->inclination = hw_acos(h[2] / h_mag);

 /* Right Ascension of Ascending Node (RAAN) */
 if (n_mag < 1e-10) {
 eph->raan = 0.0; /* For circular equatorial orbits */
 } else {
 eph->raan = hw_acos(n[0] / n_mag);
 if (n[1] < 0) {
 ...
 eph->arg_perigee = hw_atan2(e_vec[1], e_vec[0]);
 } else {
 tmp = vector_dot(n, e_vec, 3) / (n_mag * e_mag);
 if (tmp > 1.0) tmp = 1.0;
 if (tmp < -1.0) tmp = -1.0;
 eph->arg_perigee = hw_acos(tmp);
 if (e_vec[2] < 0) {
 ...
 if (tmp > 1.0) tmp = 1.0;
 if (tmp < -1.0) tmp = -1.0;
 double true_anomaly = hw_acos(tmp);
 if (r[1] < 0) {
 ...
 double cos_E = (e_mag + hw_cos(true_anomaly)) / (1.0 + e_mag * hw_cos(true_anomaly));
 double sin_E = (sqrt(1.0 - e_mag * e_mag) * hw_sin(true_anomaly)) / (1.0 + e_mag * hw_cos(true_anomaly));
 double E = hw_atan2(sin_E, cos_E);

 /* Convert eccentric anomaly to mean anomaly */
 eph->mean_anomaly = E - e_mag * hw_sin(E);
 }
...
void compute_expected_measurements(Spacecraft *sc, Spacecraft satellites[], int num_satellites,
 ...
 double azimuth = hw_atan2(enu[0], enu[1]); /* North = 0, East = pi/2 (standard nav azimuth) */
 double elevation = hw_atan2(enu[2], horizontal_distance);
 ...
 * Numerical approximation: */
 ...
 az2 = hw_atan2(temp_enu[0], temp_enu[1]);
 el2 = hw_atan2(temp_enu[2], temp_horiz);

 temp_sc_ecef[j] -= 2.0 * delta;
 ecef_to_enu(sat_ecef, temp_sc_ecef, temp_enu);

 temp_horiz = sqrt(temp_enu[0]*temp_enu[0] + temp_enu[1]*temp_enu[1]);
 az1 = hw_atan2(temp_enu[0], temp_enu[1]);
 el1 = hw_atan2(temp_enu[2], temp_horiz);
 ...
 */
 ...
 double nt = n * dt;
 double cos_nt = hw_cos(nt);
 double sin_nt = hw_sin(nt);
...
int evaluate_constellation_health(Spacecraft *sc, Spacecraft satellites[], int num_satellites,
 ...
 double elevation = PI/2.0 - hw_acos(cos_nadir < -1.0 ? -1.0 : (cos_nadir > 1.0 ? 1.0 : cos_nadir));
...
int main() {
 ...
 constellation[s].state.position[0] = orbit_r * hw_cos(angle);
 constellation[s].state.position[1] = orbit_r * hw_sin(angle);
 ...
 nav_constellation[s].state.position[0] = nav_orbit_r * hw_cos(angle);
 nav_constellation[s].state.position[1] = nav_orbit_r * hw_sin(angle);
