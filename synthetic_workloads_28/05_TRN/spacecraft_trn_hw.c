...
/*****************************************************************************
 * Terrain-Relative Navigation Algorithm — HW COMBINED VARIANT
 *
 ...
 * First transform along rows, then along columns. */
 ...
 sum_re += patch[i][n] * hw_cos(angle);
 sum_im -= patch[i][n] * hw_sin(angle);
 }
 re_row[i][k] = sum_re;
 im_row[i][k] = sum_im;
 }
 ...
 double cos_a = hw_cos(angle);
 double sin_a = hw_sin(angle);
...
double frequency_cross_correlation(double ref_patch[][DFT_PATCH_DIM],
 ...
 sr += ref_patch[i][n] * hw_cos(a);
 si -= ref_patch[i][n] * hw_sin(a);
 }
 re_tmp[i][k] = sr;
 im_tmp[i][k] = si;
 }
 ...
 double ca = hw_cos(a), sa = hw_sin(a);
 ...
 for (int n = 0; n < cols; n++) {
 double a = 2.0 * M_PI * k * n / cols;
 sr += sensor_patch[i][n] * hw_cos(a);
 si -= sensor_patch[i][n] * hw_sin(a);
 }
 re_tmp[i][k] = sr;
 im_tmp[i][k] = si;
 }
 ...
 sr += re_cross[u][v] * hw_cos(a) - im_cross[u][v] * hw_sin(a);
 }
 }
...
void initialize_terrain_map(TerrainMap* map) {
 ...
 double elevation = 100.0 * hw_sin(3.0 * x) * hw_cos(2.0 * y);

 // Add some ridges
 elevation += 50.0 * pow(hw_sin(5.0 * x * y), 2);
 ...
 // Surface roughness
 map->roughness[i][j] = 0.1 + 0.2 * fabs(hw_sin(10.0 * x * y));

 // Surface albedo (reflectivity)
 map->albedo[i][j] = 0.3 + 0.2 * hw_sin(7.0 * x) * hw_cos(8.0 * y);
 }
 }
}
...
/**
 * 3x3 Matrix multiplication — HW accelerated via SYS_MMACD
 * C = A · B computed as C += A × B (systolic) (B loaded directly (systolic, no transpose))
 */
void matrix_multiply_3x3(double A[3][3], double B[3][3], double C[3][3]) {
 const long stride = 24; /* 3 doubles × 8 bytes */
 /* SYS_MMACD computes C += A × B (systolic), so transpose B first */
 ...
 */
void matrix_multiply_9x9(double A[9][9], double B[9][9], double C[9][9]) {
 /* ── Optimized: pre-transpose B → direct MLDS/MSDS for 9×9 ── */
 /* Zero result */
 memset(C, 0, sizeof(double) * 9 * 9);

 const long stride_n = 72; /* 9 * sizeof(double) */
 ...
 */
void matrix_multiply_6x6(double A[6][6], double B[6][6], double C[6][6]) {
 /* ── Optimized: pre-transpose B → direct MLDS/MSDS for 6×6 ── */
 /* Zero result */
 memset(C, 0, sizeof(double) * 6 * 6);

 const long stride_n = 48; /* 6 * sizeof(double) */
...
/**
 * 3x3 Matrix inversion — HW accelerated via MINVD
 */
void matrix_inverse_3x3(double A[3][3], double Ainv[3][3]) {
 const long stride = 24; /* 3 doubles × 8 bytes */
 asm volatile(
...
double random_gaussian(double mean, double stddev) {
 ...
 double z0 = sqrt(-2.0 * log(u1)) * hw_cos(2.0 * M_PI * u2);
