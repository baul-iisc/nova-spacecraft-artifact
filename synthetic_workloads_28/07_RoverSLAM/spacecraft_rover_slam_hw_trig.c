 ...
 */
/*****************************************************************************
 * Planetary Rover SLAM Algorithm — HW TRIG VARIANT
 *
 * Author : Boul Chandra Garai
 * Target : RISC-V (RV64GC) with Matrix Extension Accelerator
 * Variant: CORDIC trigonometric accelerator (sin/cos/atan2 → HW)
...
void matrixMatrixBlockMultiply(Matrix* A, Matrix* B, Matrix* C) {
 ...
 int bs = MATRIX_BLOCK_SIZE;

 // Compute block multiplication
 for (int bi = 0; bi < A->rows; bi += bs) {
 for (int bj = 0; bj < B->cols; bj += bs) {
 for (int bk = 0; bk < A->cols; bk += bs) {
 // Process one block
 for (int i = bi; i < bi + bs && i < A->rows; i++) {
 for (int j = bj; j < bj + bs && j < B->cols; j++) {
 double sum = C->data[i * C->cols + j];
 for (int k = bk; k < bk + bs && k < A->cols; k++) {
...
// Note: Uses LU decomposition; pivoting improves stability for larger dimensions
void matrixInverse(Matrix* A, Matrix* Ainv) {
...
void predictMotion(SLAMSystem* slam, double dx, double dy, double dtheta) {
 ...
 double cos_theta = hw_cos(theta);
 double sin_theta = hw_sin(theta);
...
void computeJacobians(SLAMSystem* slam, Matrix* H_x, Matrix* H_l, Vector* z_pred, Observation* obs) {
 ...
 z_pred->data[1] = normalizeAngle(hw_atan2(dy, dx) - theta); // Bearing
...
void processObservations(SLAMSystem* slam, Observation* observations, int count) {
 ...
 double lm_x = x + obs->range * hw_cos(theta + obs->bearing);
 double lm_y = y + obs->range * hw_sin(theta + obs->bearing);
 ...
 * new_size > old_size ⇒ dest offset > source offset. */
 ...
 double pred_bearing = normalizeAngle(hw_atan2(dy, dx) - theta);
...
int main(int argc, char* argv[]) {
 ...
 double true_dx = 0.5 * hw_cos(step * 0.05);
 double true_dy = 0.5 * hw_sin(step * 0.05);
 ...
 double true_bearing = normalizeAngle(hw_atan2(dy, dx) - theta);
 ...
 terrain_img[iy][ix] = hw_sin(0.3*(x+ix)) * hw_cos(0.3*(y+iy)) + 0.1*((rand()%100)/100.0);
