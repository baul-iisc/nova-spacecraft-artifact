 ...
 * Variant: HW Combined (CORDIC trig + AME 3×3 tile matrix accelerators)
...
/* A: M_dim×K_dim, B: K_dim×N_dim, C: M_dim×N_dim (packed flat) */
...
/* ------------------------------------------------------------------ */
static void extract_block(const double *M, int n_stride,
 int r0, int c0, int rows, int cols, double *out) {
 for (int i = 0; i < rows; i++)
 for (int j = 0; j < cols; j++)
 out[i * cols + j] = M[(r0 + i) * n_stride + (c0 + j)];
}
static void insert_block(double *M, int n_stride,
...
/* M = [[A,B],[C,D]], A is 3×3 */
/* Ai = MINVD(A) */
/* S = D - C×Ai×B (Schur complement) */
/* Si = inverse(S) recursive */
/* Result: E = Ai + Ai×B×Si×C×Ai, F = -Ai×B×Si, */
/* G = -Si×C×Ai, H = Si */
/* ------------------------------------------------------------------ */
static int hw_inverse_recursive(const double *M, double *M_inv, int n) {
 if (n == 1) return hw_inv1(M, M_inv);
 if (n == 2) return hw_inv2(M, M_inv);
 if (n == 3) { hw_inv3(M, M_inv); return 1; }
 ...
 extract_block(M, n, 0, 0, p, p, A_blk);
 extract_block(M, n, 0, p, p, q, B_blk);
 extract_block(M, n, p, 0, q, p, C_blk);
 extract_block(M, n, p, p, q, q, D_blk);
 ...
 /* Assemble result */
 insert_block(M_inv, n, 0, 0, p, p, E_blk);
 insert_block(M_inv, n, 0, p, p, q, F_blk);
 insert_block(M_inv, n, p, 0, q, p, G_blk);
 insert_block(M_inv, n, p, p, q, q, Si);
...
// HW-accelerated: uses MINVD for 3×3 base case, recursive Schur complement,
// and SYS_MMACD-tiled matrix multiplies for all sub-operations
void small_matrix_inverse(double A[][MAX_SMALL_MATRIX_DIM], 
 double A_inv[][MAX_SMALL_MATRIX_DIM], 
 int n) {
 ...
 // Copy strided A into contiguous flat array for HW processing
 double flat_A[MAX_SMALL_MATRIX_DIM * MAX_SMALL_MATRIX_DIM];
 double flat_inv[MAX_SMALL_MATRIX_DIM * MAX_SMALL_MATRIX_DIM];
 
 for (int i = 0; i < n; i++)
 for (int j = 0; j < n; j++)
 flat_A[i * n + j] = A[i][j];
 
 // Compute inverse using HW-accelerated recursive Schur complement
 int ok = hw_inverse_recursive(flat_A, flat_inv, n);
...
void eci_to_ecef(double eci[3], double ecef[3], double gmst) {
 double cos_gmst = hw_cos(gmst);
 double sin_gmst = hw_sin(gmst);
 ...
 *alt = fabs(z) - b;
 ...
 double theta = hw_atan2(z * a, p * b);
 
 // Calculate latitude and height using iteration
 *lat = hw_atan2(z + e2 * b * pow(sw_sin(theta), 3), p - e2 * a * pow(sw_cos(theta), 3));
 double N = a / safe_sqrt(1.0 - e2 * sw_sin(*lat) * sw_sin(*lat));
 *alt = p / sw_cos(*lat) - N;
 
 // Refine iteration
 for (int i = 0; i < 3; i++) {
 *lat = hw_atan2(z + e2 * N * sw_sin(*lat), p);
...
void state_vector_to_orbital_elements(double r[3], double v[3], double elements[6]) {
 ...
 double omega;
 if (n_mag < 1e-10) {
 omega = hw_atan2(e[1], e[0]); // For equatorial orbits, use e vector in xy-plane
 } else if (e_mag < 1e-10) {
 ...
 double nu;
 if (e_mag < 1e-10) {
 nu = hw_atan2(r[1], r[0]) - Omega; // For circular orbits, measure from RAAN
 if (nu < 0) nu += 2.0 * PI;
 } else {
 ...
 E = asinh(hw_tan(nu/2.0) * safe_sqrt((e_mag+1.0)/(e_mag-1.0)));
 } else {
 E = 2.0 * hw_atan(safe_sqrt((1.0-e_mag)/(1.0+e_mag)) * hw_tan(nu/2.0)); // Eccentric anomaly
 }
 ...
 *num_measurements = 0;
 ...
 double azimuth = hw_atan2(vector_dot(unit_vector, east), vector_dot(unit_vector, north));
