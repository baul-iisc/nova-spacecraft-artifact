 ...
 * Variant: HW Trigonometric (CORDIC accelerator for all trig functions)
 ...
 double *zenith_rad, double *azimuth_rad)
{
 ...
 double decl = 0.006918 - 0.399912 * hw_cos(gamma) + 0.070257 * hw_sin(gamma)
 - 0.006758 * hw_cos(2.0 * gamma) + 0.000907 * hw_sin(2.0 * gamma)
 - 0.002697 * hw_cos(3.0 * gamma) + 0.00148 * hw_sin(3.0 * gamma);

 /* Hour angle */
 double eot = 229.18 * (0.000075 + 0.001868 * hw_cos(gamma)
 - 0.032077 * hw_sin(gamma) - 0.014615 * hw_cos(2.0 * gamma)
 - 0.04089 * hw_sin(2.0 * gamma)); /* Equation of time (minutes) */
 ...
 /* Zenith angle */
 double cos_zen = hw_sin(lat_rad) * hw_sin(decl) + hw_cos(lat_rad) * hw_cos(decl) * hw_cos(ha);
 if (cos_zen > 1.0) cos_zen = 1.0;
 if (cos_zen < -1.0) cos_zen = -1.0;
 *zenith_rad = hw_acos(cos_zen);

 /* Azimuth angle */
 double sin_zen = hw_sin(*zenith_rad);
 if (sin_zen < 1e-10) {
 *azimuth_rad = 0.0;
 } else {
 double cos_az = (hw_sin(decl) - cos_zen * hw_sin(lat_rad)) / (sin_zen * hw_cos(lat_rad));
 if (cos_az > 1.0) cos_az = 1.0;
 if (cos_az < -1.0) cos_az = -1.0;
 *azimuth_rad = hw_acos(cos_az);
 ...
 double surface_refl[NUM_BANDS])
{
 ...
 double mu_s = hw_cos(sza_rad);
 double mu_v = hw_cos(vza_rad);
 if (mu_s < 0.01) mu_s = 0.01;
 if (mu_v < 0.01) mu_v = 0.01;

 /* Rayleigh scattering phase function */
 double cos_scat = -hw_cos(sza_rad) * hw_cos(vza_rad)
 + hw_sin(sza_rad) * hw_sin(vza_rad) * hw_cos(raa_rad);
...
{
 double cloud_center_r = IMG_ROWS / 2.0 + 3.0 * hw_sin(0.2 * timestep);
 double cloud_center_c = IMG_COLS / 2.0 + 3.0 * hw_cos(0.15 * timestep);
 double cloud_radius = 6.0 + 2.0 * hw_sin(0.1 * timestep);
