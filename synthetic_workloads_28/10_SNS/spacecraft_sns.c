...
/* Software trig wrappers — direct pass-through to libm.
 The hw build replaces these with accelerator calls. */
#define sw_sin(a) sin(a)
#define sw_cos(a) cos(a)
static inline double sw_tan(double a) { return tan(a); }
static inline double sw_asin(double a) { return asin(a); }
static inline double sw_acos(double a) { return acos(a); }
static inline double sw_atan(double a) { return atan(a); }
...
void sw_matrix_inverse(double A[MATRIX_SIZE][MATRIX_SIZE],
 ...
 int pivot_idx[MATRIX_SIZE];
 double scaling[MATRIX_SIZE];

 // Initialize extended matrix [A|I]
 for (int i = 0; i < MATRIX_SIZE; i++) {
 for (int j = 0; j < MATRIX_SIZE; j++) {
 temp[i][j] = A[i][j];
 ...
 // Find maximum element in row for scaling
 double max_val = 0.0;
 for (int j = 0; j < MATRIX_SIZE; j++) {
 double abs_val = fabs(A[i][j]);
 if (abs_val > max_val) max_val = abs_val;
 }
 scaling[i] = (max_val != 0.0) ? 1.0 / max_val : 1.0;
 }
 ...
 double scaled_val = fabs(temp[pivot_idx[i]][k]) * scaling[pivot_idx[i]];
 ...
 // Swap pivot indices
 if (max_row != k) {
 int temp_idx = pivot_idx[k];
 pivot_idx[k] = pivot_idx[max_row];
 pivot_idx[max_row] = temp_idx;
 }

 // Eliminate below
 int pivot_row = pivot_idx[k];
 for (int i = k + 1; i < MATRIX_SIZE; i++) {
 int row = pivot_idx[i];
 double factor = temp[row][k] / temp[pivot_row][k];

 // Set element to 0
 temp[row][k] = 0.0;

 // Update rest of row
 for (int j = k + 1; j < MATRIX_SIZE * 2; j++) {
 temp[row][j] -= factor * temp[pivot_row][j];
 }
 }
 }
 ...
 double factor = temp[row][j];
 for (int k = j; k < MATRIX_SIZE * 2; k++) {
 temp[row][k] -= factor * temp[pivot_idx[j]][k];
 }
 }

 // Scale row
 double scale = 1.0 / temp[row][i];
 for (int j = i; j < MATRIX_SIZE * 2; j++) {
 temp[row][j] *= scale;
 }
 }
 ...
 for (int i = 0; i < MATRIX_SIZE; i++) {
 for (int j = 0; j < MATRIX_SIZE; j++) {
 Ainv[i][j] = temp[pivot_idx[i]][j + MATRIX_SIZE];
 }
 }
}
