...
/* Forward declarations for HW matrix helpers */
static void hw_matmul_general(const double *A, const double *B, double *C,
 int M_dim, int N_dim, int K_dim);
static void hw_matmul_ABt(const double *A, const double *B, double *C,
 int M_dim, int N_dim, int K_dim);
...
// Hardware-accelerated trigonometry functions (platform-specific)
// Implemented as inline assembly or compiler intrinsics
// when targeting RISC-V custom extensions
float hw_sin(float angle) {
 // Software fallback - replaced with hardware intrinsic in accelerated variant
 return sinf(angle);
}

float hw_cos(float angle) {
 // Software fallback - replaced with hardware intrinsic in accelerated variant
 return cosf(angle);
}

float hw_atan2(float y, float x) {
 // Software fallback - replaced with hardware intrinsic in accelerated variant
 return atan2f(y, x);
}
...
void hw_matrix3x3_multiply(float *A, float *B, float *C) {
 /* AME 3×3 tile accelerator: float → double, SYS_MMACD, double → float */
 const int stride = 3 * (int)sizeof(double);
 double dA[9], dB[9], dC[9];
 for (int i = 0; i < 9; i++) dA[i] = (double)A[i];
 for (int i = 0; i < 9; i++) dB[i] = (double)B[i];
 /* SYS_MMACD uses B directly (no transpose needed) */
 asm volatile(
 "mv t0, %0\n\t"
 "mv t1, %1\n\t"
 MLDS(M0, T1, T0)
 ...
 "mv t1, %3\n\t"
 MSDS(M2, T1, T0)
 :: "r"(stride), "r"(dA), "r"(dB), "r"(dC)
 : "t0", "t1", "memory"
 );
 for (int i = 0; i < 9; i++) C[i] = (float)dC[i];
}
...
void generate_science_data_products(InstrumentControlSystem *system) {
 ...
 // Matrix multiply cov * cov -> temp (18x18) [AME tiled]
 hw_matmul_general((double*)cov, (double*)cov, (double*)temp, STATE_DIM, STATE_DIM, STATE_DIM);
 ...
 // Matrix multiply field_tensor * field_tensor -> field_result [AME tiled]
 hw_matmul_general((double*)field_tensor, (double*)field_tensor, (double*)field_result, STATE_DIM, STATE_DIM, STATE_DIM);
...
/* ------------------------------------------------------------------ */
/* HW Helper: General tiled matrix multiply C = A * B */
...
/* ------------------------------------------------------------------ */
/* HW Helper: Tiled matrix multiply C = A * B^T */
/* A is M×K, B is N×K, result C is M×N */
/* ------------------------------------------------------------------ */
static void hw_matmul_ABt(const double *A, const double *B, double *C,
 int M_dim, int N_dim, int K_dim)
{
 /* ── Optimized: C = A × B^T — both A and B loaded directly (no transpose) ── */
 memset(C, 0, (size_t)M_dim * N_dim * sizeof(double));
 ...
 /* Both A and B can be loaded directly with stride_k */
 for (int kk = 0; kk < k3a; kk += 3) {
 asm volatile(
 "mv t0, %0\n\t" "mv t1, %1\n\t"
 MLDS(M0, T1, T0)
 ...
 for (int r = 0; r < 3; r++)
 for (int kk = 0; kk < K_dim; kk++)
 C[(i + r) * N_dim + j] += A[(i + r) * K_dim + kk] * B[j * K_dim + kk];
 }

 for (int i = m3a; i < M_dim; i++)
 for (int j = 0; j < N_dim; j++)
 for (int kk = 0; kk < K_dim; kk++)
 C[i * N_dim + j] += A[i * K_dim + kk] * B[j * K_dim + kk];
}
 ...
 * Returns 0 on success, -1 if singular (API compatible with original).
 */
static int gauss_jordan_inverse_12(double A[CALIB_STATE_DIM][CALIB_STATE_DIM],
 double Ainv[CALIB_STATE_DIM][CALIB_STATE_DIM])
{
 double flat_A[CALIB_STATE_DIM * CALIB_STATE_DIM];
 double flat_Ainv[CALIB_STATE_DIM * CALIB_STATE_DIM];
 for (int i = 0; i < CALIB_STATE_DIM; i++)
 for (int j = 0; j < CALIB_STATE_DIM; j++)
 flat_A[i * CALIB_STATE_DIM + j] = A[i][j];
 int ok = hw_inverse_recursive(flat_A, flat_Ainv, CALIB_STATE_DIM);
 if (!ok) return -1;
 for (int i = 0; i < CALIB_STATE_DIM; i++)
 for (int j = 0; j < CALIB_STATE_DIM; j++)
 Ainv[i][j] = flat_Ainv[i * CALIB_STATE_DIM + j];
 return 0;
}
 ...
 double P_pred[CALIB_STATE_DIM][CALIB_STATE_DIM])
{
 ...
 /* FP = F * P (N x N) [AME tiled] */
 double FP[CALIB_STATE_DIM][CALIB_STATE_DIM];
 hw_matmul_general((double*)ekf->F, (double*)ekf->P, (double*)FP, N, N, N);

 /* P_pred = FP * F' + Q [AME tiled: FP * F^T then add Q] */
 {
 double tmp[CALIB_STATE_DIM][CALIB_STATE_DIM];
 hw_matmul_ABt((double*)FP, (double*)ekf->F, (double*)tmp, N, N, N);
 for (int i = 0; i < N; i++)
 for (int j = 0; j < N; j++)
 P_pred[i][j] = tmp[i][j] + ekf->Q[i][j];
 }
}
 ...
 const double z[CALIB_STATE_DIM])
{
 ...
 /* --- HP = H * P_pred [AME tiled] --- */
 double HP[CALIB_STATE_DIM][CALIB_STATE_DIM];
 hw_matmul_general((double*)ekf->H, (double*)P_pred, (double*)HP, N, N, N);

 /* --- S = HP * H' + R [AME tiled] --- */
 double S[CALIB_STATE_DIM][CALIB_STATE_DIM];
 {
 double tmp_S[CALIB_STATE_DIM][CALIB_STATE_DIM];
 hw_matmul_ABt((double*)HP, (double*)ekf->H, (double*)tmp_S, N, N, N);
 for (int i = 0; i < N; i++)
 for (int j = 0; j < N; j++)
 S[i][j] = tmp_S[i][j] + ekf->R[i][j];
 }
 ...
 /* --- PHt = P_pred * H' [AME tiled] --- */
 double PHt[CALIB_STATE_DIM][CALIB_STATE_DIM];
 hw_matmul_ABt((double*)P_pred, (double*)ekf->H, (double*)PHt, N, N, N);

 /* --- Kalman gain K = PHt * S_inv [AME tiled] --- */
 double K[CALIB_STATE_DIM][CALIB_STATE_DIM];
 hw_matmul_general((double*)PHt, (double*)S_inv, (double*)K, N, N, N);
 ...
 * where SEU-induced bit-flips can perturb matrix entries. */
 ...
 {
 double KH[CALIB_STATE_DIM][CALIB_STATE_DIM];
 hw_matmul_general((double*)K, (double*)ekf->H, (double*)KH, N, N, N);
 for (int i = 0; i < N; i++)
 for (int j = 0; j < N; j++)
 IKH[i][j] = ((i == j) ? 1.0 : 0.0) - KH[i][j];
 }

 /* term1 = IKH * P_pred * IKH^T [AME tiled] */
 double IKH_P[CALIB_STATE_DIM][CALIB_STATE_DIM];
 hw_matmul_general((double*)IKH, (double*)P_pred, (double*)IKH_P, N, N, N);
 double term1[CALIB_STATE_DIM][CALIB_STATE_DIM];
 hw_matmul_ABt((double*)IKH_P, (double*)IKH, (double*)term1, N, N, N);

 /* term2 = K * R * K^T [AME tiled] */
 double KR[CALIB_STATE_DIM][CALIB_STATE_DIM];
 hw_matmul_general((double*)K, (double*)ekf->R, (double*)KR, N, N, N);
 double term2[CALIB_STATE_DIM][CALIB_STATE_DIM];
 hw_matmul_ABt((double*)KR, (double*)K, (double*)term2, N, N, N);
