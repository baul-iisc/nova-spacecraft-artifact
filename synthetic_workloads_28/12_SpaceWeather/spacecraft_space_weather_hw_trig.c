...
/* Trig/math wrappers — CORDIC accelerated (double-precision HW) */
...
/**
 * In-place matrix inverse via Gauss-Jordan (n x n), for small n.
 * Returns 0 on success, -1 on singular.
 */
static int mat_inv(const double *A, double *Ainv, int n)
{
 double aug[18 * 2 * 18]; /* max 18x36 augmented row-major */
 if (n > 18) return -1;
