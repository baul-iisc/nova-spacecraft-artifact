...
#define sinf(a) ((float)hw_sin((double)(a)))
#define cosf(a) ((float)hw_cos((double)(a)))
#define atan2(y, x) hw_atan2(y, x)
#define asin(a) hw_asin(a)
...
/* ------------------------------------------------------------------ */
/* HW Helper: General double matrix multiply via SYS_MMACD 3×3 tiles */
/* ------------------------------------------------------------------ */
static void hw_matmul_double(const double *A, const double *B, double *C,
 int m, int k, int n)
{
...
void geometric_correction(SARData* sar, float geom_matrix[DOPPLER_BINS][DOPPLER_BINS]) {
 ...
 // Apply transformation using AME-tiled matrix multiplication
 // temp_block = geom_matrix × block (real and imag separately)
 static complex_float temp_block[DOPPLER_BINS][DOPPLER_BINS];
 {
 double G[DOPPLER_BINS * DOPPLER_BINS];
 double Br[DOPPLER_BINS * DOPPLER_BINS];
 double Bi[DOPPLER_BINS * DOPPLER_BINS];
 double Cr[DOPPLER_BINS * DOPPLER_BINS];
 double Ci[DOPPLER_BINS * DOPPLER_BINS];

 /* Promote float → double for AME pipeline */
 for (int i = 0; i < DOPPLER_BINS; i++)
 for (int j = 0; j < DOPPLER_BINS; j++) {
 G [i * DOPPLER_BINS + j] = (double)geom_matrix[i][j];
 Br[i * DOPPLER_BINS + j] = (double)block[i][j].real;
 Bi[i * DOPPLER_BINS + j] = (double)block[i][j].imag;
 }

 /* SYS_MMACD 3×3 tiled multiply: Cr = G × Br, Ci = G × Bi */
 hw_matmul_double(G, Br, Cr, DOPPLER_BINS, DOPPLER_BINS, DOPPLER_BINS);
 hw_matmul_double(G, Bi, Ci, DOPPLER_BINS, DOPPLER_BINS, DOPPLER_BINS);

 /* Demote double → float back to complex_float */
 for (int i = 0; i < DOPPLER_BINS; i++)
 for (int j = 0; j < DOPPLER_BINS; j++) {
 temp_block[i][j].real = (float)Cr[i * DOPPLER_BINS + j];
 temp_block[i][j].imag = (float)Ci[i * DOPPLER_BINS + j];
 }
 }
