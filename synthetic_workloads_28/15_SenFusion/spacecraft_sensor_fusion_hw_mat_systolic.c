 ...
 * Variant: HW Mat (AME 3x3 tile accelerator for matrix multiply/inverse)
...
/* -------- AME 3x3 tile helpers for matrix operations -------- */
#define P_TILE 3
...
static void hw_inv3(const double *A, double *Ainv) {
 ...
 MINVD(M2, M1)
 MSDS(M2, T2, T0)
 :: "r"(P_TILE * (int)sizeof(double)), "r"(A), "r"(Ainv)
 : "t0", "t1", "t2", "memory"
 );
}

static void hw_extract_block(const double *M, int ld,
 int r0, int c0,
 int brows, int bcols,
 double *out) {
 for (int i = 0; i < brows; i++)
 for (int j = 0; j < bcols; j++)
 out[i * bcols + j] = M[(r0 + i) * ld + (c0 + j)];
}

static void hw_insert_block(double *M, int ld,
 int r0, int c0,
 int brows, int bcols,
 const double *blk) {
 for (int i = 0; i < brows; i++)
 for (int j = 0; j < bcols; j++)
 M[(r0 + i) * ld + (c0 + j)] = blk[i * bcols + j];
}
...
/* AME-tiled contiguous multiply: C(m x n) = A(m x k) * B(k x n) */
...
/* Recursive Schur complement inverse for n x n (contiguous) */
static int hw_inverse_recursive(const double *M, double *M_inv, int n_dim) {
 ...
 hw_inv3(A_blk, Ai);
 hw_matmul_cont(C_blk, Ai, CA, q, p, p);
 hw_matmul_cont(CA, B_blk, tmp_qq, q, p, q);
 for (int i = 0; i < q * q; i++) S[i] = D_blk[i] - tmp_qq[i];

 ok = hw_inverse_recursive(S, Si, q);
 if (!ok) goto cleanup;

 hw_matmul_cont(Ai, B_blk, AB, p, p, q);
 hw_matmul_cont(AB, Si, F_blk, p, q, q);
 for (int i = 0; i < p * q; i++) F_blk[i] = -F_blk[i];

 hw_matmul_cont(Si, CA, G_blk, q, q, p);
 for (int i = 0; i < q * p; i++) G_blk[i] = -G_blk[i];

 for (int i = 0; i < p * q; i++) neg_F[i] = -F_blk[i];
 hw_matmul_cont(neg_F, CA, tmp_pp, p, q, p);
 ...
 * HW Mat variant: SYS_MMACD 3x3 tiled with stride support
 */
void matrix_multiply_generic(double *A, double *B, double *C, 
 ...
 /* Pre-transpose B (n×p with stride ldb) into flat BT (p×n contiguous) */
 ...
 const long stride_a = lda * (long)sizeof(double);
 const long stride_bt = n * (long)sizeof(double);
 const long stride_c = ldc * (long)sizeof(double);

 int m3a = (m / 3) * 3;
 int p3a = (p / 3) * 3;
 int n3a = (n / 3) * 3;

 for (int i = 0; i < m3a; i += P_TILE) {
 for (int j = 0; j < p3a; j += P_TILE) {
 asm volatile(MZERO(M2));

 for (int kk = 0; kk < n3a; kk += P_TILE) {
 asm volatile(
 "mv t0, %0\n\t" "mv t1, %1\n\t"
 MLDS(M0, T1, T0)
 "mv t0, %2\n\t" "mv t1, %3\n\t"
 MLDS(M1, T1, T0)
 SYS_MMACD(M2, M0, M1)
 :: "r"(stride_a), "r"(&A[i * lda + kk]),
 "r"(stride_bt), "r"(&B[j * n + kk])
 : "t0", "t1", "memory"
 );
 }
 ...
 C[(i + r) * ldc + j] += A[(i + r) * lda + kk] * B[kk * ldb + j];
 }
...
/**
 * Generic matrix inversion (SW Gauss-Jordan).
 * Uses leading dimensions for proper striding.
 * Copies strided input to a contiguous buffer, inverts, copies back.
 */
void matrix_inverse_generic(double *A, double *Ainv, int n, int lda, int ldainv) {
 /* Copy strided input to contiguous n x n buffer */
 double *Acont = (double *)flight_malloc_impl(n * n * sizeof(double));
 double *Icont = (double *)flight_malloc_impl(n * n * sizeof(double));
 if (!Acont || !Icont) {
 FLIGHT_LOG("Memory allocation failed in matrix_inverse_generic\n");
 flight_free_impl(Acont); flight_free_impl(Icont);
 return;
 }
 for (int i = 0; i < n; i++)
 for (int j = 0; j < n; j++)
 Acont[i*n + j] = A[i*lda + j];

 /* SW Gauss-Jordan inverse with partial pivoting: Acont -> Icont */
 for (int i = 0; i < n*n; i++) Icont[i] = 0.0;
 for (int i = 0; i < n; i++) Icont[i*n + i] = 1.0;

 for (int col = 0; col < n; col++) {
     int pivot = col;
     double max_val = fabs(Acont[col*n + col]);
     for (int r = col + 1; r < n; r++) {
         double v = fabs(Acont[r*n + col]);
         if (v > max_val) { max_val = v; pivot = r; }
     }
     if (max_val < 1e-12) {
         /* Singular / ill-conditioned — return identity as a safe fallback */
         for (int i = 0; i < n*n; i++) Icont[i] = 0.0;
         for (int i = 0; i < n; i++) Icont[i*n + i] = 1.0;
         break;
     }
     if (pivot != col) {
         for (int j = 0; j < n; j++) {
             double t = Acont[col*n + j];
             Acont[col*n + j] = Acont[pivot*n + j];
             Acont[pivot*n + j] = t;
             t = Icont[col*n + j];
             Icont[col*n + j] = Icont[pivot*n + j];
             Icont[pivot*n + j] = t;
         }
     }
     double diag = Acont[col*n + col];
     if (fabs(diag) < 1e-18) diag = (diag >= 0.0 ? 1e-18 : -1e-18);
     double inv_diag = 1.0 / diag;
     for (int j = 0; j < n; j++) {
         Acont[col*n + j] *= inv_diag;
         Icont[col*n + j] *= inv_diag;
     }
     for (int r = 0; r < n; r++) {
         if (r == col) continue;
         double f = Acont[r*n + col];
         if (fabs(f) < 1e-18) continue;
         for (int j = 0; j < n; j++) {
             Acont[r*n + j] -= f * Acont[col*n + j];
             Icont[r*n + j] -= f * Icont[col*n + j];
         }
     }
 }
 ...
 for (int i = 0; i < n; i++)
 for (int j = 0; j < n; j++)
 Ainv[i*ldainv + j] = Icont[i*n + j];

 flight_free_impl(Acont);
 flight_free_impl(Icont);
}
