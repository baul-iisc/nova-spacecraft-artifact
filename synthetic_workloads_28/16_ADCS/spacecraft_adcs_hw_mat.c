...
void matrix_multiply(LargeMatrix *A, LargeMatrix *B, LargeMatrix *C) {
 ...
 double *Af = (double *)A->data;
 double *Bf = (double *)B->data;
 double *Cf = (double *)C->data;
 double *BT = (double *)flight_malloc_impl((size_t)n * n * sizeof(double));
 memset(Cf, 0, (size_t)n * n * sizeof(double));
 ...
 MLDS(M1, T1, T0)
 SYS_MMACD(M2, M0, M1)
 :: "r"(stride_n), "r"(&Af[i * n + kk]),
 "r"(stride_n), "r"(&Bf[kk * n + j])
 : "t0", "t1", "memory"
 );
 }
 if (n3a < n) {
 ...
 a_pad[r * 3 + c] = Af[(i + r) * n + n3a + c];
 b_pad[r * 3 + c] = Bf[(j + r) * n + n3a + c];
 }
 asm volatile(
 "mv t0, %0\n\t" "mv t1, %1\n\t"
 MLDS(M0, T1, T0)
 ...
 Cf[(i + r) * n + j] += Af[(i + r) * n + kk] * Bf[kk * n + j];
 }
 for (int i = n3a; i < n; i++)
 for (int j = 0; j < n; j++)
 for (int kk = 0; kk < n; kk++)
 Cf[i * n + j] += Af[i * n + kk] * Bf[kk * n + j];
}
