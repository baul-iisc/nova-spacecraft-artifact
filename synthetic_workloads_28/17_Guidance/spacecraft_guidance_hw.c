 ...
 * HW Combined (CORDIC trig + AME 3x3 tile matrix accelerator)
 *
...
static double pdg_BT_P[CONTROL_DIM * STATE_DIM];
static double pdg_BT_P_B[CONTROL_DIM * CONTROL_DIM];
static double pdg_BT_P_A[CONTROL_DIM * STATE_DIM];
...
/* ---- AME 3x3 tile helper functions for HW matrix acceleration ---- */

/* 3x3 inverse via MINVD instruction */
static void hw_inv3(double *A, double *Ainv) {
 const int P = 3;
...
/* Extract sub-block from flat row-major matrix */
static void hw_extract_block(double *M, int ld, int r0, int c0,
 double *B, int brows, int bcols) {
 for (int i = 0; i < brows; i++)
 for (int j = 0; j < bcols; j++)
 B[i * bcols + j] = M[(r0 + i) * ld + (c0 + j)];
}

/* Insert sub-block into flat row-major matrix */
static void hw_insert_block(double *B, int brows, int bcols,
 double *M, int ld, int r0, int c0) {
 for (int i = 0; i < brows; i++)
 for (int j = 0; j < bcols; j++)
 M[(r0 + i) * ld + (c0 + j)] = B[i * bcols + j];
}

/* Small-matrix inverse: always use SW Gauss-Jordan (disable MINVD path) */
static void hw_inv1(double *A, double *Ainv, int n) {
 if (0 && n <= 3) {
 double pad[9] = {0}, ipad[9] = {0};
 for (int i = 0; i < n; i++)
 for (int j = 0; j < n; j++)
 pad[i * 3 + j] = A[i * n + j];
 for (int i = n; i < 3; i++) pad[i * 3 + i] = 1.0;
 hw_inv3(pad, ipad);
 for (int i = 0; i < n; i++)
 for (int j = 0; j < n; j++)
 Ainv[i * n + j] = ipad[i * 3 + j];
 return;
 }
 /* Fallback: Gauss-Jordan for 4..6 */
 int nn = n;
 double *aug = (double *)flight_malloc_impl(nn * 2 * nn * sizeof(double));
 for (int i = 0; i < nn; i++) {
 for (int j = 0; j < nn; j++) aug[i*2*nn+j] = A[i*nn+j];
 for (int j = 0; j < nn; j++) aug[i*2*nn+nn+j] = (i==j)?1.0:0.0;
 }
 for (int i = 0; i < nn; i++) {
 int piv=i; double mx=fabs(aug[i*2*nn+i]);
 for (int k=i+1;k<nn;k++){double v=fabs(aug[k*2*nn+i]);if(v>mx){mx=v;piv=k;}}
 if(piv!=i) for(int j=0;j<2*nn;j++){double t=aug[i*2*nn+j];aug[i*2*nn+j]=aug[piv*2*nn+j];aug[piv*2*nn+j]=t;}
 double d=aug[i*2*nn+i]; if(fabs(d)<1e-15) d=1e-15;
 for(int j=0;j<2*nn;j++) aug[i*2*nn+j]/=d;
 for(int k=0;k<nn;k++){if(k==i)continue;double f=aug[k*2*nn+i];for(int j=0;j<2*nn;j++)aug[k*2*nn+j]-=f*aug[i*2*nn+j];}
 }
 for (int i=0;i<nn;i++) for(int j=0;j<nn;j++) Ainv[i*nn+j]=aug[i*2*nn+nn+j];
 flight_free_impl(aug);
}

/* Schur-complement single-level inverse */
static void hw_inv2(double *M, double *Minv, int n) {
 int h = n/2, rest = n - h;
 double *Ap = (double*)flight_malloc_impl(h*h*sizeof(double));
 double *Bp = (double*)flight_malloc_impl(h*rest*sizeof(double));
 double *Cp = (double*)flight_malloc_impl(rest*h*sizeof(double));
 double *Dp = (double*)flight_malloc_impl(rest*rest*sizeof(double));
 double *Ai = (double*)flight_malloc_impl(h*h*sizeof(double));
 double *S = (double*)flight_malloc_impl(rest*rest*sizeof(double));
 double *Si = (double*)flight_malloc_impl(rest*rest*sizeof(double));
 double *w1 = (double*)flight_malloc_impl(rest*h*sizeof(double));
 double *w2 = (double*)flight_malloc_impl(h*rest*sizeof(double));
 double *w3 = (double*)flight_malloc_impl(h*h*sizeof(double));

 hw_extract_block(M,n,0,0,Ap,h,h);
 hw_extract_block(M,n,0,h,Bp,h,rest);
 hw_extract_block(M,n,h,0,Cp,rest,h);
 hw_extract_block(M,n,h,h,Dp,rest,rest);

 hw_inv1(Ap, Ai, h);

 /* S = D - C * Ai * B */
 {double *CA=(double*)flight_malloc_impl(rest*h*sizeof(double));
 for(int i=0;i<rest;i++) for(int j=0;j<h;j++){double s=0;for(int k=0;k<h;k++) s+=Cp[i*h+k]*Ai[k*h+j];CA[i*h+j]=s;}
 for(int i=0;i<rest;i++) for(int j=0;j<rest;j++){double s=0;for(int k=0;k<h;k++) s+=CA[i*h+k]*Bp[k*rest+j];S[i*rest+j]=Dp[i*rest+j]-s;}
 flight_free_impl(CA);}

 hw_inv1(S, Si, rest);

 /* w1 = Si * C * Ai */
 {double *CA2=(double*)flight_malloc_impl(rest*h*sizeof(double));
 for(int i=0;i<rest;i++) for(int j=0;j<h;j++){double s=0;for(int k=0;k<h;k++) s+=Cp[i*h+k]*Ai[k*h+j];CA2[i*h+j]=s;}
 for(int i=0;i<rest;i++) for(int j=0;j<h;j++){double s=0;for(int k=0;k<rest;k++) s+=Si[i*rest+k]*CA2[k*h+j];w1[i*h+j]=s;}
 flight_free_impl(CA2);}

 /* w2 = Ai * B * Si */
 {double *AB=(double*)flight_malloc_impl(h*rest*sizeof(double));
 for(int i=0;i<h;i++) for(int j=0;j<rest;j++){double s=0;for(int k=0;k<h;k++) s+=Ai[i*h+k]*Bp[k*rest+j];AB[i*rest+j]=s;}
 for(int i=0;i<h;i++) for(int j=0;j<rest;j++){double s=0;for(int k=0;k<rest;k++) s+=AB[i*rest+k]*Si[k*rest+j];w2[i*rest+j]=s;}
 flight_free_impl(AB);}

 /* w3 = Ai + w2 * C * Ai */
 {double *TC=(double*)flight_malloc_impl(h*h*sizeof(double));
 for(int i=0;i<h;i++) for(int j=0;j<h;j++){double s=0;for(int k=0;k<rest;k++) s+=w2[i*rest+k]*Cp[k*h+j];TC[i*h+j]=s;}
 for(int i=0;i<h;i++) for(int j=0;j<h;j++){double s=0;for(int k=0;k<h;k++) s+=TC[i*h+k]*Ai[k*h+j];w3[i*h+j]=Ai[i*h+j]+s;}
 flight_free_impl(TC);}

 for(int i=0;i<h*rest;i++) w2[i]=-w2[i];
 for(int i=0;i<rest*h;i++) w1[i]=-w1[i];

 hw_insert_block(w3,h,h,Minv,n,0,0);
 hw_insert_block(w2,h,rest,Minv,n,0,h);
 hw_insert_block(w1,rest,h,Minv,n,h,0);
 hw_insert_block(Si,rest,rest,Minv,n,h,h);

 flight_free_impl(Ap);flight_free_impl(Bp);flight_free_impl(Cp);flight_free_impl(Dp);
 flight_free_impl(Ai);flight_free_impl(S);flight_free_impl(Si);flight_free_impl(w1);flight_free_impl(w2);flight_free_impl(w3);
}

/* Square n x n multiply using SYS_MMACD 3x3 tiles */
static void hw_matmul_cont(double *A, double *B, double *C, int n) {
...
/* Recursive Schur complement inverse (AME-accelerated) */
static void hw_inverse_recursive(double *A, double *Ainv, int n) {
 if (n <= 3) { hw_inv1(A, Ainv, n); return; }
 if (n <= 6) { hw_inv2(A, Ainv, n); return; }
 int h = n/2, rest = n - h;
 double *Asub=(double*)flight_malloc_impl(h*h*sizeof(double));
 double *Bp =(double*)flight_malloc_impl(h*rest*sizeof(double));
 double *Cp =(double*)flight_malloc_impl(rest*h*sizeof(double));
 double *Dp =(double*)flight_malloc_impl(rest*rest*sizeof(double));
 double *Ai =(double*)flight_malloc_impl(h*h*sizeof(double));
 ...
 hw_extract_block(A,n,0,0,Asub,h,h);
 hw_extract_block(A,n,0,h,Bp,h,rest);
 hw_extract_block(A,n,h,0,Cp,rest,h);
 hw_extract_block(A,n,h,h,Dp,rest,rest);

 hw_inverse_recursive(Asub, Ai, h);
 ...
 hw_insert_block(w3,h,h,Ainv,n,0,0);
 hw_insert_block(w2,h,rest,Ainv,n,0,h);
 hw_insert_block(w1,rest,h,Ainv,n,h,0);
 hw_insert_block(Si,rest,rest,Ainv,n,h,h);

 flight_free_impl(Asub);flight_free_impl(Bp);flight_free_impl(Cp);flight_free_impl(Dp);
 flight_free_impl(Ai);flight_free_impl(S);flight_free_impl(Si);flight_free_impl(w1);flight_free_impl(w2);flight_free_impl(w3);
}

// Matrix multiplication: C = A * B where A is m x n, B is n x p, C is m x p
// HW Combined variant: SYS_MMACD 3x3 tiled
 ...
 * No dynamic memory allocation occurs inside this function. */
 ...
 double* BT_P = pdg_BT_P;
 double* BT_P_B = pdg_BT_P_B;
 double* BT_P_A = pdg_BT_P_A;
