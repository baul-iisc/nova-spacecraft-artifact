...
/* Direct math library calls (hw build substitutes accelerator intrinsics) */
#define sw_sin(angle) sin(angle)
#define sw_cos(angle) cos(angle)
#define sw_asin(angle) asin(angle)
#define sw_acos(angle) acos(angle)
#define sw_tan(angle) tan(angle)
#define sw_atan(angle) atan(angle)
 ...
 */
void sw_matrix_multiply(double *A, double *B, double *C,
 int m, int n, int p)
{
...
/* Recursive Schur complement inverse */
static void hw_inverse_recursive(const double *A, double *R, int n) {
 ...
 int k = n / 2, rest = n - k;
 double w1[36*36], w2[36*36], w3[36*36];
 double Ai[36*36], S[36*36], Si[36*36];
...
#define MAX_INV_DIM (MEAS_DIM * NUM_SPACECRAFT)
 ...
 static double Areg[MAX_INV_DIM * MAX_INV_DIM];
 for (int i = 0; i < n * n; i++) Areg[i] = A[i];
 for (int i = 0; i < n; i++) Areg[i * n + i] += 1e-12;
 hw_inverse_recursive(Areg, A_inv, n);
}
