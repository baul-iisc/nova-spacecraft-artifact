...
Matrix multiplyMatrices(Matrix *a, Matrix *b) {
 ...
 for (int r = 0; r < P; r++)
 for (int c = 0; c < P; c++)
 At[r * P + c] = a->data[i + r][kk + c];
 /* Gather B transposed tile: Bt[r][c] = b->data[kk+c][j+r] */
 for (int r = 0; r < P; r++)
 for (int c = 0; c < P; c++)
 Bt[r * P + c] = b->data[kk + c][j + r];
 asm volatile(
 "mv t0, %0\n\t" "mv t1, %1\n\t"
 MLDS(M0, T1, T0)
 "mv t0, %2\n\t" "mv t1, %3\n\t"
...
static void hw_inverse_recursive(const double *A, double *R, int n) {
 ...
 int k = n / 2, rest = n - k;
 double w1[MATRIX_SIZE*MATRIX_SIZE], w2[MATRIX_SIZE*MATRIX_SIZE], w3[MATRIX_SIZE*MATRIX_SIZE];
 double Ai[MATRIX_SIZE*MATRIX_SIZE], S[MATRIX_SIZE*MATRIX_SIZE], Si[MATRIX_SIZE*MATRIX_SIZE];
 ...
 * HW Mat variant: Schur complement recursive with AME 3x3 tiles
 */
Matrix inverseMatrix(Matrix *m) {
 Matrix result;
 const int N = MATRIX_SIZE;

 /* Convert struct to flat array */
 static double flat_in[MATRIX_SIZE * MATRIX_SIZE];
 static double flat_out[MATRIX_SIZE * MATRIX_SIZE];
 for (int i = 0; i < N; i++)
 for (int j = 0; j < N; j++)
 flat_in[i * N + j] = m->data[i][j];

 /* Regularise diagonal for numerical stability */
 for (int i = 0; i < N; i++)
 flat_in[i * N + i] += 1e-12;

 hw_inverse_recursive(flat_in, flat_out, N);

 /* Convert flat result back to struct */
 for (int i = 0; i < N; i++)
 for (int j = 0; j < N; j++)
 result.data[i][j] = flat_out[i * N + j];
