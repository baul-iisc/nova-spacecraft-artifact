...
/* Redirect float trig to CORDIC hardware */
...
/* DFT frequency analysis on DWT subbands for quality assessment */
void compute_subband_dft(float *coeffs, int len, float *magnitude) {
 ...
 re += coeffs[j] * compute_cosine(angle);
 im -= coeffs[j] * compute_sine(angle);
 }
