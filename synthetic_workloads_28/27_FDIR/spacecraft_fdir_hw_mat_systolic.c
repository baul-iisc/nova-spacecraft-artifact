...
void matrix_multiply(const Matrix* a, const Matrix* b, Matrix* result) {
 ...
 const long s24 = 24;
 int m3a = (M / 3) * 3, n3a = (N / 3) * 3, k3a = (K / 3) * 3;
...
/* AME 3×3 tiled matrix multiplication using MLDS/SYS_MMACD/MSDS */
void matrix_tile_multiply_3x3(const Matrix* a, const Matrix* b, Matrix* result) {
 const int P = MATRIX_TILE_SIZE; /* 3 */
 const int stride = P * (int)sizeof(double);
 int tiles_rows_a = a->rows / P;
 int tiles_cols_a = a->cols / P;
 int tiles_cols_b = b->cols / P;
 double w1[9], w2[9], w3[9];
 ...
 /* Extract A tile into w1 (row-major 3×3) */
 for (int r = 0; r < P; r++)
 for (int c = 0; c < P; c++)
 w1[r*P+c] = a->data[(i*P+r)*a->cols + (k*P+c)];
 /* Extract B tile transposed: w2[r][c] = B[k*P+c][j*P+r] */
 for (int r = 0; r < P; r++)
 for (int c = 0; c < P; c++)
 w2[r*P+c] = b->data[(k*P+r)*b->cols + (j*P+c)];
 asm volatile(
 "mv t0, %0\n\t" "mv t1, %1\n\t"
 MLDS(M0, T1, T0)
 "mv t1, %2\n\t"
 ...
 for (int r = 0; r < P; r++)
 for (int c = 0; c < P; c++)
 result->data[(i*P+r)*result->cols + (j*P+c)] = w3[r*P+c];
 }
 }
}
